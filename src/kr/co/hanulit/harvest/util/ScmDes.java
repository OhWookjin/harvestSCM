...
public final class ScmDes
{

	//protected static LogWriter logger = LoggingService.getLogger(ScmDes.class);
    ...
    static void main(String args[])
    {
        ...
        try
        {
            ...
            String s1 = "19327992837113116";
            des.setKey("a;sd9-98()*;lkKAS9:LKjS(n(*_", s1);
            ScmDesApplet.setKey("a;sd9-98()*;lkKAS9:LKjS(n(*_", s1);
            String s2 = des.encrypt(s);
            String s3 = ScmDesApplet.encrypt(s);
            
            trace(s2);
            trace(s3);
            trace(des.decrypt(s2));
            trace(des.decrypt(s3));
        }
        catch(Exception exception)
        {
            trace("Exception:" + exception.getMessage());
        }
        return;
    }
	...
	public String cmdencrypt(String s)
    {
        String s1 = null;
        try
        {
            byte abyte0[] = s.getBytes("euc-kr");
        ...
        throws Exception
    {
        ...
        try
        {
            byte abyte0[] = ScmByteArray.fromString(s);
            //int i = abyte0.length;
            abyte0 = decrypt(abyte0);
            s1 = new String(abyte0, "EUC-KR");
        }
        ...
        throws Exception
    {
        ...
        if(m_TableStatus)
        {
            byte byte0 = 8;
            if(i % byte0 != 0)
                throw new Exception("Des: length of Data must be in multiple of 8");
            DES3(D_DECRYPT, abyte0, j, i);
            int k;
            for(k = 0; k < i && abyte0[k] != 0; k++);
            int l = k;
            abyte1 = new byte[l];
            System.arraycopy(abyte0, 0, abyte1, 0, l);
        } else
        {
            throw new Exception("Des: key has not been set");
        }
        return abyte1;
    }
    ...
    {
        ScmCASHA1 casha1 = new ScmCASHA1();
        //byte abyte3[] = new byte[4];
        byte abyte4[] = new byte[20];
        casha1.Update(abyte0, 0, i);
        casha1.Update(abyte1, 0, j);
        casha1.Final(abyte4, 0);
    ...
    {
        m_TableStatus = false;
        Arrays.fill(m_EncryptKn1, 0);
        Arrays.fill(m_DecryptKn1, 0);
        Arrays.fill(m_EncryptKn2, 0);
        Arrays.fill(m_DecryptKn2, 0);
        Arrays.fill(m_EncryptKn3, 0);
        Arrays.fill(m_DecryptKn3, 0);
    ...
    private static void dumpByteArray(byte abyte0[])
    {
        dumpByteArray(abyte0, "");
    }

    private static void dumpByteArray(byte abyte0[], String s)
    {
        System.out.print(s + "{");
    ...
    private static void dumpByteArray(byte abyte0[], int i, int j, String s, FileWriter filewriter)
    {
        try
        {
            filewriter.write(s);
            for(int k = 0; k < j; k++)
                filewriter.write(abyte0[i + k] + ",");
    ...
    private static void dumpIntArray(int ai[], String s, FileWriter filewriter)
        throws IOException
    {
        filewriter.write(s);
        for(int i = 0; i < ai.length; i++)
            filewriter.write(ai[i] + ",");
	...
	*/
    ...
    {
        //logger.systemLog("hcore.util.Des: " + s);
    }
    
    private static final String CRYPT_PASSPHRASE = "leavinonasoutherntrainonlyyesterdayyoulied";
    private static final String CRYPT_SEED = "thisstuffisbulletproofyeahthatsright";
